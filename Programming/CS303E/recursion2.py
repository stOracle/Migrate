...
#  Description: Second set of recursion problems
...
def groupSum5(start, nums, target):
  if (start == len(nums)):
    ...
    return (nums[start] == target) or (target == 0)
  else:
  ...
  if (start == len(nums)):
    ...
    for i in range (len(nums[start:]) - 1):
      if (nums[start + i] == nums[start + i + 1]):
        count += 1
      elif (nums[start + i] != nums[start + i + 1]):
        break
...
def splitOdd10Helper(a, b, idx):
  if (idx == len(a)):
    ...
    c = b[:]
    b.append(a[idx])
    if ((sum(b) % 10 == 0) and (sum(c) % 2 == 1) or ((sum(c) % 10 == 0) and (sum(b) % 2 == 1))):
      return True
    else:
...
def split53Helper(a, b, idx):
  if (idx == len(a)):
    return (sum(a) == 2 * sum(b)) and (sum(b) == sum(c))
  else:
    c = b[:]
    if (b == []):
      return split53Helper(a, b, idx + 1) or split53Helper(a, c, idx + 1)
    if (b[-1] % 5 == 0):
      c = c[:-1]
      b.append(a[idx])
      return split53Helper(a, b, idx + 1) or split53Helper(a, c, idx + 1)
