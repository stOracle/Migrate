...
#  Description: A list of recursive exercises
...
def countAbc(st):
  if (len(st) < 3):
    ...
    if ((st[:3] == "abc") or (st[:3] == "aba")):
      return 1 + countAbc(st[1:])
    else:
...
def strDist(st, sub):
  if (len(st) < len(sub)):
    ...
    if ((st[:len(sub)] == sub) and (st[-len(sub):] == sub)):
      return len(st)
    elif (st[:len(sub)] != sub):
