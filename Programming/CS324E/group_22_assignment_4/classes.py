...
class Cube(Figure):
    ...
    def collides_sphere(self, other):
        ...
        #     return False
        if self.orig.distance(other.center) < other.R + self.sideL/2:
            return True
        else:
            return False
    ...
    def update(self):
        ...
        if (self.x + self.sideL/2 >= 500) or (self.x - self.sideL/2 <= -500):
            self.x -= self.vx
            self.vx = -self.vx
        if (self.y + self.sideL/2 >= 500) or (self.y - self.sideL/2 <= -500):
             self.y -= self.vy             
             self.vy = -self.vy*self.L          
        if (self.z + self.sideL/2 >= 500) or (self.z - self.sideL/2 <= -500):
            self.z -= self.vz
            self.vz = -self.vz
        ...
        self.thX = (self.thX + self.omegaX)%(2*PI)
        self.thY = (self.thY + self.omegaY)%(2*PI)
        self.thZ = (self.thZ + self.omegaZ)%(2*PI)
